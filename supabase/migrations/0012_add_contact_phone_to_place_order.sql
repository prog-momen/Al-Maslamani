drop function if exists public.place_order_from_cart(text, text, public.payment_method, numeric, text);
...
begin
  ...
  insert into public.addresses (
    ...
    coalesce(nullif(contact_phone_input, ''), nullif(profile_row.phone, ''), '-'),
    'غير محدد',
...
grant execute on function public.place_order_from_cart(text, text, public.payment_method, numeric, text, text) to authenticated;
