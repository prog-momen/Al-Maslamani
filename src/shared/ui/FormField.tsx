...
export function FormField({ label, error, iconType, secureTextEntry, className, ...props }: FormFieldProps) {
  ...
  const Icon = iconType ? FormIcons[iconType] : null;
  ...
  return (
    <View className="mb-4">
      ...
      >
        {error ? (
          <FormIcons.Error color="#D32F2F" size={20} className="ml-3" />
        ) : Icon ? (
        ...
        {isPassword && (
          <Pressable onPress={() => setIsPasswordVisible(!isPasswordVisible)} className="mr-2 px-2 py-2 hit-slop-10">
            {isPasswordVisible ? <FormIcons.EyeOff color="#757575" size={20} /> : <FormIcons.Eye color="#757575" size={20} />}
          </Pressable>
        )}
      </View>
      {error ? <Text className="text-brand-error text-right mt-1 font-tajawal-medium text-[13px]">{error}</Text> : null}
    </View>
  );
}
